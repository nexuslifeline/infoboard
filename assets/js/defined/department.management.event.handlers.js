...
showNavigation();
            ...
            var bindEventHandlers=(function(){
                ...
                $('#tbl_info_list tbody').on('click','button[name="edit_info"]',function(){
                    ...
                    infoModalModule.setDetails({
                        "department_id"           :       row.find('td:eq(0) input[type="checkbox"]').val(),
                        "department_title"        :       row.find('td:eq(0) input[type="checkbox"]').attr('data-department-title'),
                         "department_desc"        :       row.find('td').eq(2).html(),
                        "status"            :       row.find('td:eq(0) input[type="checkbox"]').attr('data-status'),
                                ...
                                "columnDefs": [
                                    {
                                        ...
                                        'render': function(data, type, full, meta){
                                            ...
                                            'value="'+data.split('|')[0]+'"' +
                                            'data-department-title="'+data.split('|')[1]+'"' +
                                           /// 'data-area-desc="'+data.split('|')[2]+'"' +
...

                                    {
                                        'targets': [4],
                                        'render': function (data, type, full, meta){
                                            var btn_edit='<button class="btn btn-white btn-sm" name="edit_info"  style="margin-left:-15px;" data-toggle="tooltip" data-placement="top" title="Edit Department"><i class="fa fa-pencil"></i> </button>';
    ...
    var infoModalModule=(function(){
                    ...
                    if(_mode=="new"){ //if current mode is new
                        createNewInfo()
                            ...
                            .success(function(response){ //if request is successful
                                ...
                                var data=[row.hidden,row.department_title,row.department_desc,row.date_created,''];
        ...
        var createNewInfo=function(){
            ...
            var department_desc = $('.summernote').code();
               
               serialData.push({
                name:"department_desc",value: department_desc
            });
            ...
            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"DepartmentManagement/ActionSaveDepartmentInfo",
                "data":serialData
            });
        ...
        var updateInfo=function(){
            ...
            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"DepartmentManagement/ActionUpdateDepartmentInfo",
                "data":serialData
            });
        ...
        var removeInfo=function(){
            ...
            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"DepartmentManagement/ActionDeleteDepartmentInfo",
                "data":serialData
            });
        ...
        var setInfoModalDetails=function(data){
            ...
            $('input[name="department_title"]').val(data.department_title);
            $('textarea[name="department_desc"]').val(data.department_desc);
            $('.summernote').code(data.department_desc);
    ...
    var _btnNew='<button class="btn btn-white btn-sm"  id="btn_new_info" data-toggle="modal" data-target="#info_modal" data-placement="left" title="Create New Department" ><i class="fa fa-users"></i> Create New Department</button>';
